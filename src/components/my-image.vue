    ...
    <el-dialog :visible.sync="dialogVisible" width="750px">
      <el-tabs v-model="activeName" type="card">
        <el-tab-pane label="素材库" name="image">
          ...
          <!-- 列表 -->
          <div class="img_list">
            <div :class="{selected:selectedImageUrl===item.url}" class="img_item"  @click="selectedImage(item.url)" v-for="item in images" :key="item.id">
              <img :src="item.url" />
            </div>
          </div>
          <!-- 分页 -->
...
export default {
  // value在发布文章的时候是没有值的,编辑的时候才有图片数据
  props: ['value'],
  name: 'my-image',
  data () {
    return {
